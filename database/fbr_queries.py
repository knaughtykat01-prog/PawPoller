"""All SQL CRUD functions for the Furbooru (fbr) analytics database.

Public Philomena read JSON API. Poll-only, tracks the connected
user's own uploads (tags=user:<username>). Engagement shape:
  - score (score.total, can be negative), favorites_count (fav_count),
    comments_count (comment_count).
  - submission_id is the Philomena image id as TEXT.
...
instead of `views` (fbr exposes no view count).
"""
...
def get_fbr_submission(conn: sqlite3.Connection, submission_id: str) -> dict | None:
    row = conn.execute("SELECT * FROM fbr_submissions WHERE submission_id = ?", (submission_id,)).fetchone()
    return dict(row) if row else None


def get_all_fbr_submissions(conn: sqlite3.Connection, sort_by: str = "score", order: str = "desc", account_id: int | None = None) -> list[dict]:
    ...
    sql = "SELECT * FROM fbr_submissions" + (f" WHERE {where}" if where else "")
...
def get_fbr_snapshots(conn: sqlite3.Connection, submission_id: str,
                       start: str | None = None, end: str | None = None) -> list[dict]:
    sql = "SELECT * FROM fbr_snapshots WHERE submission_id = ?"
...
def get_fbr_aggregate_snapshots(conn: sqlite3.Connection, start: str | None = None,
    ...
    sql = ("SELECT polled_at, SUM(score) as score, SUM(favorites_count) as favorites_count, "
           "SUM(comments_count) as comments_count FROM fbr_snapshots")
    params: list[Any] = []
    conditions = []
    if start:
...
def get_fbr_comparison_snapshots(conn: sqlite3.Connection, submission_ids: list[str],
    ...
    sql = f"SELECT * FROM fbr_snapshots WHERE submission_id IN ({placeholders})"
...
def start_fbr_poll_log(conn: sqlite3.Connection, account_id: int = 0) -> int:
    cur = conn.execute(
        "INSERT INTO fbr_poll_log (started_at, status, account_id) VALUES (datetime('now'), 'running', ?)",
        (account_id,))
    conn.commit()
    return cur.lastrowid


def finish_fbr_poll_log(conn: sqlite3.Connection, log_id: int, status: str,
                         submissions_found: int = 0, snapshots_inserted: int = 0,
                         error_message: str | None = None, duration_seconds: float = 0) -> None:
    conn.execute(
        """UPDATE fbr_poll_log SET finished_at=datetime('now'), status=?, submissions_found=?,
...
def get_fbr_last_poll(conn: sqlite3.Connection) -> dict | None:
    row = conn.execute("SELECT * FROM fbr_poll_log ORDER BY started_at DESC LIMIT 1").fetchone()
    return dict(row) if row else None


def get_fbr_poll_log(conn: sqlite3.Connection, limit: int = 50) -> list[dict]:
    rows = conn.execute("SELECT * FROM fbr_poll_log ORDER BY started_at DESC LIMIT ?", (limit,)).fetchall()
    return [dict(r) for r in rows]


# -- Furbooru Summary ------------------------------------------------------------

def get_fbr_summary(conn: sqlite3.Connection, account_id: int | None = None) -> dict:
    ...
    totals = conn.execute(
        ...
        "FROM fbr_submissions" + w,
        wp,
    ).fetchone()
    totals = dict(totals)

    top_scored = conn.execute(
        "SELECT submission_id, title, score FROM fbr_submissions" + w + " ORDER BY score DESC LIMIT 5",
        wp,
    ).fetchall()

    top_faved = conn.execute(
        "SELECT submission_id, title, favorites_count FROM fbr_submissions" + w + " ORDER BY favorites_count DESC LIMIT 5",
        wp,
    ).fetchall()
...
def get_fbr_growth_rates(conn: sqlite3.Connection) -> dict:
    totals = conn.execute(
    ...
    for label, hours in [("24h", 24), ("7d", 168), ("30d", 720)]:
        row = conn.execute(
            """SELECT SUM(score) as score, SUM(favorites_count) as favorites_count,
               ...
               FROM fbr_snapshots WHERE polled_at = (
                   SELECT polled_at FROM fbr_snapshots
...
def get_fbr_submission_growth_rates(conn: sqlite3.Connection, submission_id: str) -> dict:
    sub = conn.execute(
        "SELECT score, favorites_count, comments_count FROM fbr_submissions WHERE submission_id = ?",
        (submission_id,),
    ).fetchone()
    if not sub:
        return {}
    ...
    for label, hours in [("24h", 24), ("7d", 168), ("30d", 720)]:
        row = conn.execute(
            """SELECT score, favorites_count, comments_count
               FROM fbr_snapshots WHERE submission_id = ? AND polled_at <= datetime('now', ? || ' hours')
               ORDER BY polled_at DESC LIMIT 1""",
...
def get_fbr_submission_deltas(conn: sqlite3.Connection) -> dict[str, dict]:
    rows = conn.execute(
