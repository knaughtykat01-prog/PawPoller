...
so a Mastodon commenter ``@rhys@some.other.instance`` matched our own
``@rhys@our.instance`` and a stranger's comment was silently marked as ours.
...
def own_handles(conn, platform: str,
    ...
    - Mastodon's ``acct`` is a bare local name for home-instance users and
      ``user@host`` for remote ones. Storing ``rhys@our.instance`` alone would
      never match our own home-instance comments, which arrive as ``rhys``. So
      the bare local part is added too — a match on it still means a
      home-instance user of that name, i.e. us. A remote
      ``rhys@other.social`` matches neither entry.
    - Bluesky handles never contain '@' (``rhys.bsky.social``), so an
