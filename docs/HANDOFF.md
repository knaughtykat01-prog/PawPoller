...
**Current version:** 2.14.4 (security + robustness pass)
**Deployed to:** GCP instance `pawpoller` (zone `us-east1-c`) — server on 2.14.3 as of 02:33 UTC. 2.14.4 fixes are pure code; deploy with `pawupdate` when ready.
**GitHub release:** https://github.com/knaughtykat01-prog/PawPoller/releases/tag/v2.14.3 — tag includes the Windows zip artifact. 2.14.4 hasn't been tagged yet (no new desktop functionality to ship to end users — server-side robustness only; bundle into the next feature release).
