...
**Current version:** 2.16.9 (Mobile Mode Phase 5 sweep + backlog cleanup. 2.16.4–2.16.7 closed out the mobile-mode work after a Playwright audit at 440×956 against the live prod dashboard: CSP hash fix that had been silently dropping every `data-mobile` rule for four releases, page-header padding-left so titles clear the hamburger, stats-grid `!important` to beat the inline JS style, page-header wrap so multi-button action rows flow on mobile, settings-tabs `max-width:100%` so the existing scroll-x finally engages, and `.main-content { max-width:100vw }` as belt-and-braces. 2.16.8 closed three deferred drive-bys from the round-2 prod live-monitor: SameSite=Strict→lax fixes a periodic 401 burst pattern, `/favicon.ico` exempted from auth middleware, `/api/health` now returns `version`. 2.16.9 collapsed BUG-016's 9× polling fan-out into a single `/api/poll/all-progress` endpoint — 88% fewer requests during a sync, single failure path keeps the DevTools console clean.)
