...
**Deployed to:** GCP instance `pawpoller` (zone `us-east1-c`), running 2.26.2 — in sync with master. **2026-06-10 incident:** GCP billing lapsed and Google TERMINATED the VM (polling down for up to ~2 weeks). Billing re-enabled + VM restarted same day; container came back healthy on its restart policy, first poll cycle clean. The ephemeral external IP changed on restart: 35.243.213.49 → **35.231.162.181** — anything pointing at the old IP (browser bookmarks, desktop pairing `posting_server_url`) needs updating. Consider reserving a static IP to stop this recurring.
