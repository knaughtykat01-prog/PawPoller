...
downvotes, may be negative) — identical to e621, so the backend was generated by mechanical transform of the e621 files
and the frontend mirrors e621's score dashboard. Full stack: `database/fbr_schema.sql`+`fbr_queries.py`
(fbr_submissions/snapshots/poll_log, score metric, `account_id` in fresh schema), `polling/fbr_poller.py` (orchestrator +
`multi_account`), `routes/fbr_api.py` (`/api/fbr/*` mounted), session-check registered (username-only "configured" gate),
full frontend (registry `pollOnly:true`/dashboard/subs/detail/compare/connect form/~14 API methods/`fbrTopList`+
`fbrSubmissionsTable`+`fbrPollLogTable`). Connect = **username only** (API key optional — raises rate cap; vaulted).
Feeds digest/insights/tag-perf/repost-radar via the platform maps. **Posting deferred** (Philomena cookie-scrape upload,
exactly as e621 posting was 2.104→2.118). +4 tests. **Also fixed pre-existing FN wiring gaps** (missing from `_dashResync`
/ bulk `TRIGGERS`+`RESYNCS` / hub `fetchers` / `prefixMap` / `_pollingAuth`) — added both `fn` and `fbr`. **Expansion
roadmap:** next fediverse batch (reuse `mast`: Pleroma/GoToSocial/Firefish/Pixelfed), Aryion, Piczel/Picarto,
SubscribeStar; Reddit own track.

**Prior — 2.200.0 — FurryNetwork = the 18th platform (poll+post).** First of the platform
...
`resolve_account_credentials` is_default→unsuffixed = what the poster's `_resolve_creds("ik")` reads). `app.js`:
token input on the connect form + a Posting status row & Save-token control on the connected panel; `API.ikSetToken`.
+3 tests. **The token is still user-supplied (from their logged-in Itaku session).**

**Prior — 2.190.2 — Masterpieces "＋ Add variant" (upload a render straight in).**
