...
**Current version:** 2.14.5 (audit-debt refactor pass)
**Deployed to:** GCP instance `pawpoller` (zone `us-east1-c`) — server on 2.14.3 as of 02:33 UTC. 2.14.4 + 2.14.5 changes are pure code; deploy with `pawupdate` when ready.
**GitHub release:** https://github.com/knaughtykat01-prog/PawPoller/releases/tag/v2.14.3 — tag includes the Windows zip artifact. 2.14.4 + 2.14.5 haven't been tagged yet (server-side hardening + refactor only; bundle into the next feature release).
