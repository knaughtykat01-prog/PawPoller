...
**Current version:** 2.26.3 (**Fix: SQLite "database is locked" poll failures + blank error messages.** Post-outage log sweep found six intermittent IB poll failures over ten days, two `database is locked` plus an IK casualty on the first post-restart cycle. Root cause: four pollers (IB, FA, SqW, AO3) opened an implicit sqlite3 write transaction with a snapshot upsert and then held it across network awaits before committing — AO3's kudos fetches at 12s pacing could hold the write lock for minutes, starving every other poller past the 30s `busy_timeout`. Caught red-handed in the log: IB wrote at 01:08:32 then awaited a 60s faving-users fetch; IK tried to write at 01:08:32 and died at exactly 01:09:02 (busy_timeout + 30s). Fix: commit immediately after the snapshot upsert before any conditional fetch awaits, in all four; IB also commits per-iteration after comment upserts. Principle now documented inline: never hold an open write transaction across an await. The other seven pollers already write-then-commit after all fetching — safe. Second fix: `httpx.ReadTimeout` and the timeout family stringify to "", producing literal blank `Poll ib failed: ` log lines and empty dashboard/Telegram error fields. New `describe_error(e)` in `polling/notifications.py` (str(e) or qualified type name fallback) applied at all 11 pollers' error handlers, server.py's orchestrator loop, and telegram.py's `send_poll_error`. 91 tests green.)
**Earlier:** 2.26.2 (**Fix: CI build unblocked — bump `softprops/action-gh-release` v2 → v3.** The 2.26.1 tag-push triggered the Build & Release workflow which then failed three consecutive times at "Set up job" on both `build-windows` and `build-linux` with `Failed to download archive 'https://codeload.github.com/softprops/action-gh-release/{zip,tar.gz}/3bb12739c298aeb8a4eeaf626c5b8d85266b0e65' after 1 attempts. An action could not be found at the URI '...'`. Direct `curl` of the archive returned HTTP 200 + valid zip and the GitHub API confirmed `v2` still resolved to that commit — so the archive was reachable; the failure was inside GitHub Actions' Marketplace lookup for the pinned commit, not a codeload outage. Three immediate-fail reruns ruled out the documented transient softprops "Server Error" pattern (which `gh run rerun --failed` recovered on second pass for the 2.25.0 build). `softprops/action-gh-release@v3.0.0` shipped 2026-04-12 — Node 20 → Node 24 runtime bump, no input/API changes. Bumped both `@v2` refs in `.github/workflows/build.yml` (build-windows line 77, build-linux line 149) to `@v3`. The broken v2.26.1 tag stays in tag history — no force-push to re-point it. v2.26.2 carries the 2.26.1 FA error-message fix plus this CI unblock.)
**Earlier:** 2.26.1 (**Fix: FA poll error messages no longer look like PawPoller bugs.** Live polling on 2026-05-26 surfaced every FA cycle (every ~4h) failing with a raw `httpx.HTTPStatusError` dump: `FA poll failed: Server error '500 Internal Server Error' for url 'https://faexport.spangle.org.uk/user/knaughtykat/gallery.json?page=1&full=1'`. Probe showed FAExport itself was healthy (root renders, latest `v2025.12.1`) but its gallery endpoint was 500ing for every user (verified against `fender` as a known-good control); FA was responding fine to our IP. So the breakage was in FAExport's own scraper session against FA — nothing PawPoller could fix, but the dashboard / Telegram / DB error_message all looked like PawPoller bugs because they showed the unredacted 500-URL string. New `FAExportUpstreamError` exception + `_humanize_fa_error()` helper in `polling/fa_poller.py` pattern-match `httpx.HTTPStatusError` by URL host (`faexport.`) and status: **5xx** → "FAExport upstream error (N) — third-party proxy faexport.spangle.org.uk could not fetch data from FurAffinity. Not a PawPoller bug; will retry next cycle." + link to `Deer-Spangle/faexport/issues`; **429** → shared-bucket-pressure message (complements 2.23.2's in-client retry which handles transient 429s); **404** → username-may-be-wrong message; anything else passes through unchanged. Top-level except now translates before reporting, re-raises with `raise friendly from e` to preserve original traceback. Server.py's orchestrator log picks up the friendly message automatically via `str(result)`.)
**Earlier:** 2.26.0 (**Feature: in-app Uninstall flow + Windows Search integration polish.** Closes the "I deleted the .AppImage / extracted folder, but the data and autostart entries are still on my machine" gap. New **Settings → General → Danger zone → Uninstall PawPoller** button. Two-step modal: GET `/api/settings/uninstall/plan` returns the detected install type + paths (rendered in a monospace block in the dialog so the user can sanity-check); POST `/api/settings/uninstall` kicks off the cleanup. Requires `confirm: "UNINSTALL"` in the body as a guard. **The Inno Setup installer was already correctly registering with Windows Search / Apps & features / Control Panel via `HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\{AppId}_is1`** (AppId + UninstallDisplayName + UninstallDisplayIcon all set in 2.24.0) — right-clicking the Start Menu search result for "PawPoller" already showed Open / Pin / Uninstall. This release wires the in-app button to delegate to the same `unins000.exe /SILENT` so both paths share one uninstaller. New `uninstall.py` module (~290 LOC): `InstallType` enum (WINDOWS_INSTALLER detected by unins000.exe presence next to PawPoller.exe / WINDOWS_PORTABLE / LINUX_APPIMAGE detected by APPIMAGE env var / DEV / UNKNOWN); `detect()` returns an UninstallPlan dataclass with app_path + data_dir + autostart_target + has_keyring_key; `execute()` synchronously removes autostart via `config.set_run_on_startup(False)` + removes keyring vault key via `keyring.delete_password("PawPoller", "vault_key")`, then asynchronously spawns a detached per-OS cleanup script. Inno installer → `unins000.exe /SILENT`; Windows portable → .bat with `rmdir /S /Q` on install dir + data dir (defensive `if exist "{install}\PawPoller.exe"` guard); Linux AppImage → .sh with `rm -f $APPIMAGE` + `rm -rf` data dir + autostart .desktop; Dev mode → refuses to delete the source tree (won't nuke a developer's working copy), cleans data + autostart only. Detached spawn via `os.startfile` (Windows) or `subprocess.Popen(start_new_session=True, std*=DEVNULL)` (Linux). Server schedules `os._exit(0)` for 2s after responding via `asyncio.get_event_loop().call_later` so the response flushes before shutdown. Frontend Danger zone accordion in app.js (~120 LOC) with red-tinted styling, three checkboxes (app / data / autostart), and a literal-text "UNINSTALL" confirm input. **Known limitation:** vault key in OS keyring is only cleaned up via the in-app Uninstall button; users uninstalling via Windows Search → Uninstall (or `rm` the AppImage manually) will leave a 64-char hex value in Windows Credential Manager / Secret Service — tiny, no security impact (encrypted vault file is gone too), but a stray entry. macOS uninstall raises RuntimeError until the native app ships.)
**Earlier:** 2.25.0 (**Feature: Linux desktop support (AppImage).** PawPoller now ships native builds for both Windows and Linux. The Linux build is a single-file `.AppImage` — distro-independent, no install required, double-click to run on Ubuntu 22.04+, Fedora 37+, Debian 12+, Arch. The codebase had only four Windows-isms; each got a per-OS shim rather than `if sys.platform` sprinkles. **Autostart** (`config.py`) now branches between HKCU registry (Windows) and XDG autostart `.desktop` file at `~/.config/autostart/PawPoller.desktop` (Linux, honoured by every major DE); macOS is wired but logs a warning until the launch-agent plist branch lands. **Desktop notifications** (`polling/notifications.py:show_toast`) branch between winotify (Windows) and shell-out to `notify-send` with `--app-name=PawPoller --expire-time=8000` (Linux); silently no-ops if notify-send isn't on PATH. **PyInstaller spec** (`pawpoller.spec`) `hiddenimports` branches via `_PLATFORM_HIDDEN_IMPORTS` so each OS bundles its pystray backend (`_win32` / `_appindicator+_gtk` / `_darwin`). **pywebview backend on Linux** (`main.py`) switched to Qt (`webview.start(gui='qt')`) because GTK+WebKit2 bindings don't bundle cleanly via PyInstaller/AppImage; Qt6+QtWebEngine are pip-installable and ship their own native libs. **requirements.txt** uses env markers (`winotify ; sys_platform == "win32"`, `PyQt6 + PyQt6-WebEngine ; sys_platform == "linux"`) so one file handles all OSes. **AppImage build**: new `installer/build-appimage.sh` constructs the AppDir layout from `dist/PawPoller/` (PyInstaller --onedir), writes AppRun launcher + .desktop entry + icon (from existing `assets/tray_icon.png`), then runs `appimagetool`. New `build-linux` job in `.github/workflows/build.yml` runs on `ubuntu-22.04` (GLIBC 2.35 floor for forward-compat), installs WeasyPrint runtime libs + libnotify-bin + Qt6 platform plugin deps (libgl1/libegl1/libxcb-*/libxkbcommon-x11-0/libdbus-1-3/libnss3/libxcomposite1/libxdamage1/libxrandr2/libasound2 for QtWebEngine), runs PyInstaller, runs build-appimage.sh, uploads + attaches `PawPoller-*-x86_64.AppImage` to the GitHub Release. **Auto-updater** (`updater.py`): new `_pick_update_asset()` picks per-OS (`.zip` on Windows for the slot-replace path; `.AppImage` on Linux for in-place replace; the Windows installer `.exe` intentionally NOT chosen by the in-app updater — that's for fresh installs). New `_apply_update_linux()` writes a tiny bash script that sleeps 2s, `mv`'s the new AppImage over the path in `$APPIMAGE` env var, chmod +x, exec — spawned detached via `subprocess.Popen(start_new_session=True)`. `download_update()` now derives the temp filename from URL basename so the extension is preserved (helps log triage). Marketing site + README updated; ROADMAP_PUBLIC.md got a new "Cross-platform desktop" section with Linux marked done and **macOS detailed as planned** — same shape work plus `.app`/`.dmg` packaging plus the open Apple Developer cert / notarization decision (signing $99/yr cert + notarization tooling, or accept Gatekeeper friction).)
**Earlier:** 2.24.0 (**Feature: Windows install wizard.** Replaces the "download zip, extract, run exe" first-run experience with a proper Inno Setup single-file installer. New file `installer/PawPoller.iss` (~90 LOC): per-user install by default (no UAC), optional Start Menu / desktop shortcuts, optional "Run on Windows startup" task that writes the `HKCU\…\Run` entry (mirrors the in-app toggle), "Launch now" tick-on-finish with `skipifsilent` so future silent auto-updates don't pop the GUI, fixed AppId GUID for proper in-place upgrades, AppVersion injected at build time from `config.py:APP_VERSION` via `/DMyAppVersion` so there's no version drift between code and installer. Uninstaller registers under Add or Remove Programs and offers (default No) to delete `%APPDATA%\PawPoller` on removal so most uninstalls preserve data for the next install. Best-effort `taskkill /F /IM PawPoller.exe` runs before file deletions to avoid "in use" errors. CI changes in `.github/workflows/build.yml`: new "Read app version" step parses `APP_VERSION` from config.py into a job output; new "Build installer" step calls `iscc` (Inno Setup 6 is pre-installed on `windows-latest` runners as of 2026-05-20, with `choco install innosetup` fallback); `PawPoller-Setup-*.exe` uploaded as artifact alongside the existing zip; GitHub Release attaches both. Existing zip is preserved as a portable option. **Code signing intentionally skipped** — SmartScreen warns either way without an Authenticode cert (~$200-400/yr), so the installer doesn't make trust worse than the zip already does; defer until non-technical users hit it at scale. **Tag-cut reminder:** installer only ships on `v*` tag pushes — pushing to master alone doesn't produce one. Cut `v2.24.0` when ready to publish.)
**Earlier:** 2.23.3 (**Fix: AO3 form-fetch 525s killed edits that should have retried.** Live `Drumheller_Detour` AO3 update on 2026-05-20 06:47:15 hit a single Cloudflare 525 (origin SSL handshake fail) on `GET /works/85113586/edit` and the entire edit raised `RuntimeError: AO3: Could not load edit form (status 525)` instantly, bouncing into the retry queue. `_get_page()` already retries 3× with backoff on 525 (the documented "~1 in 5 from datacenter IPs" mitigation), but six form-fetch GETs were using `self._http.get(...)` directly and bypassing it: `edit_work`, `edit_chapter`, `create_chapter`, `find_work_skin_by_title`, `create_work_skin`, `update_work_skin`. Same log also showed `find_work_skin_by_title` silently returning None on its own 525 → cryptic empty-error `"Work skin creation failed for Drumheller_Detour:"` warning. Routed all six through `_get_page` in `clients/ao3/client.py`; callers now check `html is None` instead of `resp.status_code != 200` and report "transient fetch failure" to distinguish from hard 4xx. Delete-confirm GETs (`delete_work`) intentionally left alone — admin-only, not in the hot path.)
**Earlier:** 2.23.2 (**Two fixes shipped together.** **(a) Publish-check action log was overlapping the action panel.** User screenshot showed `Recent actions (N)` header + log entries painting on top of the "Save as draft (where supported)" checkbox row inside the cell drawer. Root cause: the action-log placeholder was a **sibling** of the detail panel inside `.publish-check-body` (flex column, `gap: 16px`); with the log populated and the detail content tall, the gap collapsed in some browser layouts and the log overlaid the action panel options. Fix in `frontend/js/publish_check.js`: move the placeholder **inside** the action panel as the last child after `.publish-action-result`, and re-call `_renderActionLog()` after every `detail.innerHTML = html` so the per-session `_actionLog` closure repopulates the fresh placeholder. Three edits: drop the sibling div from `_renderMatrix`; emit it inside `_renderActionPanel`; call `_renderActionLog()` in `_showDetail` after the innerHTML assignment. **(b) FAExport 429 → Retry-After + small pacing bump.** Live FA poll cycle on 2026-05-20 hit two back-to-back FAExport 429s within 2ms — first on a submission detail call (logged + skipped, batch continued), then on the watchers fetch (caught by the broad except in `polling/fa_poller.py:392`, cycle survived but watcher list was lost for the cycle). Same per-IP throttle window; pacing inside the detail loop ran out before the watchers call, and FAExport's bucket is shared across all `faexport.spangle.org.uk` users — so a 429 there is usually someone else's traffic, not ours. Picking the right shape for the fix mattered: bumping pacing alone doesn't help with shared-bucket pressure, and AO3's full backoff-cache pattern (2.22.6/2.22.10) is the wrong shape — AO3 has a fixed-window Rack::Attack counter where requests inside the window extend the punishment, FAExport is a thin proxy without that pathology. Honouring `Retry-After` is the minimum correct change. New `_get_with_retry(url, *, params, max_retries=1, max_sleep=60)` on `FAClient` wraps every FAExport `GET`. On 429: reads `Retry-After` (defaults 30s, clamped to [1, 60]), sleeps, retries once; non-429 passed through untouched so callers still own raise_for_status. Five callers converted: `get_gallery_page`, `get_submission_detail`, `get_submission_comments`, `get_user_profile`, `get_watchers_page`. Direct-FA cookie-validation path unchanged — different throttling shape, wasn't what fired. Also bumped `FA_REQUEST_DELAY_SECONDS = 1.0 → 1.5` as cheap insurance — doesn't help against shared-bucket pressure but cuts our own contribution by a third; ~5s extra per ten-submission detail batch, invisible at the 240-min cadence.)
...
**Deployed to:** GCP instance `pawpoller` (zone `us-east1-c`), running 2.26.3 — in sync with master. **2026-06-10 incident:** GCP billing lapsed and Google TERMINATED the VM (polling down for up to ~2 weeks). Billing re-enabled + VM restarted same day; container came back healthy on its restart policy, first poll cycle clean. The ephemeral external IP changed on restart: 35.243.213.49 → **35.231.162.181** — anything pointing at the old IP (browser bookmarks, desktop pairing `posting_server_url`) needs updating. Consider reserving a static IP to stop this recurring. **FAExport outage (FA polling dead since ~2026-05-26):** every JSON endpoint on faexport.spangle.org.uk 500s with `error_type: unknown_http` (web UI fine, all users, all client IPs — their scraper session against FA is broken). Reported upstream 2026-06-10 as [Deer-Spangle/faexport#129](https://github.com/Deer-Spangle/faexport/issues/129); check there before re-investigating FA poll errors. If it stays dead long-term, the fallback play is direct-FA polling via cookie auth (the posting path already talks to FA directly).
...
| **In-app Uninstall + Windows Search integration** | 2.26.0 | New `uninstall.py` module detects install type (Inno installer / Windows portable / Linux AppImage / dev) and spawns a per-OS detached cleanup script. Inno path delegates to `unins000.exe /SILENT` — same code path as Windows Search → Uninstall, Settings → Apps & features → Uninstall, Control Panel → Programs and Features → Uninstall (which were already wired by Inno's 2.24.0 metadata). Portable zip + AppImage paths build their own .bat/.sh that wait for the process to exit then `rmdir`/`rm -f` the install dir + (optionally) data dir + autostart entry. Dev mode refuses to delete the source tree. Vault key in OS keyring cleaned up synchronously before the script spawns. New `GET /api/settings/uninstall/plan` + `POST /api/settings/uninstall` endpoints. Frontend: Settings → General → Danger zone accordion with red-tinted styling, three checkboxes, and a literal-text "UNINSTALL" confirm input. |
...
5. **Testing checklists** — all QA artefacts live under `qa/`:
   - `qa/TESTING_CHECKLIST_WEBAPP.html` — 566 rows × 52 sections (last updated 2026-05-22 for v2.26.0 catch-up), browser/Docker/server flavour. localStorage key `pawpoller_test_webapp`. CSV exports as `pawpoller_test_webapp.csv`. Section numbers 17–18 intentionally vacant (Goals + Tags surfaces removed in 2.16.14).
   - `qa/TESTING_CHECKLIST_NATIVE.html` — 638 rows × 63 sections (last updated 2026-05-22). Windows + Linux desktop builds (PyInstaller exe / AppImage + pywebview + tray + Inno installer + in-app Uninstall). localStorage key `pawpoller_test_native`. CSV exports as `pawpoller_test_native.csv`.
