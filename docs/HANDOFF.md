...
**Earlier:** 2.20.5 (**Fix: PDF rendering no longer blocks the dashboard event loop.** First bulk Regenerate All with "include PDF" ticked froze the dashboard — page loads hung, regen SSE stalled, polling ticks skipped, CPU at 150% for the whole ~22min run. Root cause: `editor.pdf_generator.html_to_pdf()` is sync CPU-bound (WeasyPrint), called directly from the async regenerate endpoint — holds the GIL for the full 30-80s render per story while the asyncio loop can't service anything else. Fix: wrap each `html_to_pdf` call in `asyncio.to_thread(...)` so renders run on Python's threadpool executor; event loop releases on the await and stays responsive. Bulk regen still serialises PDFs one at a time but the dashboard stays alive throughout. Two call sites updated in `routes/editor_api.py` (full-story + per-chapter loop). EPUB is also sync but completes in ~200ms so no thread hop needed; other format generators are likewise fast enough.)
**Earlier:** 2.20.4 (**Fix: Single-chapter EPUB fallback + chapter-marker injection script.** First bulk Regenerate All in production surfaced Extra_Credit failing EPUB with "No chapters found in MASTER.md after body anchor" — that story's MASTER.md has no `# Chapter X` headings; chapter boundaries live only in `chapters.json` as line ranges (legacy from before PawPoller's editor existed). Two fixes: (1) defensive fallback in `editor/epub_generator.py:_split_into_chapters` — when no `# ` headings after `<!-- @body -->` but body has content, treat the whole body as one synthetic chapter using the story title. Multi-chapter stories with proper headings unaffected (only triggers when chapters list is empty after the walk). (2) New `scripts/inject_chapter_markers.py` one-shot rescue tool: reads `chapters.json` + `Chapters/Markdown/Chapter_*.md` files, anchors each chapter's start by matching its first content line in MASTER.md, inserts `# Chapter N: Title` headings at the right positions. Backs up MASTER.md → .bak; sorts chapter files by extracted N (so 10 lands after 9, not between 1 and 2); idempotent (refuses to operate if markers already present). Ran against Extra_Credit, injected 10 markers, pawsync'd to server, regen now produces all 9 formats cleanly (EPUB 208 KB, 10 chapters split + converted).)
**Earlier:** 2.20.3 (**Fix: AO3 chapter-creation duplicate-draft loop + cancel button now sticks.** Two stop-the-bleeding fixes. (1) `clients/ao3/client.py:create_chapter` had the identical preview-page bug as `create_work` (fixed in 2.20.1): POST `/works/{id}/chapters` returns the preview page inline at the same URL when submitted via Preview button, no chapter ID in the URL. PawPoller raised "Could not extract chapter_id", retried, each retry created a fresh draft. v2.20.1's fix to `create_work` made it look like progress because work creation now succeeded, but chapter creation immediately failed and respawned a draft per retry cycle. Same fix shape applied — body-scan fallback gated by success markers (Draft/Chapter was successfully created, Preview Work / Edit Work / Edit Chapter titles). (2) Cancel button silently did nothing because `cancel_queue_item` matched only `status='pending'` — by the time a user clicked cancel the row was usually `processing` already. Even worse, when cancel DID hit a pending row, the scheduler's failure path overwrote `cancelled` with `pending` on the next failure (scheduler.py line 165 calls update_queue_status unconditionally). Both fixed in `database/posting_queries.py`: cancel matches pending/retrying/processing/failed; all UPDATE statements in `update_queue_status` now include `AND status != 'cancelled'` so cancel is sticky even mid-flight. New `cancel_all_for(conn, platform=, story_name=)` helper for bulk-cancel by filter. **Known pending issue (deferred)**: `manager._schedule_retry` (from `post_story` failure path) creates a fresh queue row AND the scheduler resets the original row — so one failure spawns two retry attempts. Not blocking now that create_chapter is fixed but worth tackling next session.)
**Earlier:** 2.20.2 (**Fix: AO3 full-story posts used Clean HTML instead of SquidgeWorld OTW format.** The AO3 poster's `_read_full_story_html()` had `HTML/<Story>_Clean.html` as the first preference and `SquidgeWorld/Chapter_*.html` concatenation only as fallback — but AO3 is an OTW Archive site like SqW; the SqW per-chapter HTML carries the OTW chapter markers, warning-icon glyph, and semantic anchors AO3 parses, while Clean HTML is the generic shape for Inkbunny/Weasyl/etc. Per-chapter path (`_read_chapter_html`) already used SqW correctly — only full-story was wrong. Fix: invert preference. SqW concatenation is primary, Clean HTML is last-resort for pre-SqW archives. Anything regenerated since 2.18.x will hit the new primary path. **Stale zombies on AO3**: the user has ~5 duplicate Hypnotic_Claim drafts from the 2.20.1 retry loop. Drafts page is currently 429'd from those attempts; cleanup deferred (user can delete manually via AO3's Drafts UI in 2 min, or the next session can retry the scripted cleanup after the rate limit clears).)
**Earlier:** 2.20.1 (**Fix: AO3 work creation produced silent duplicate drafts.** When publishing `Hypnotic_Claim` to AO3, every retry showed `Could not extract work ID from https://archiveofourown.org/works` — but the POST `/works` actually succeeded server-side each time, leaving a fresh duplicate draft on the user's AO3 account (zombie drafts auto-expire 2026-06-10 per AO3's 30-day TTL). Root cause in `clients/ao3/client.py:create_work`: code only checked the response *URL* for `/works/{id}`. When the form is submitted via the **Preview** button, AO3 renders the Preview Work page inline at `/works` without redirecting — so the URL stays ID-less while the body carries the work ID in every action button. Fix: when the URL match fails, look for success markers in the body (`Draft was successfully created` flash, `<title>Preview Work`, or `<title>Edit Work`) and scan for `/works/(\d+)` as a strict fallback. URL-redirect path still wins when AO3 does redirect; body-scan only fires when success-marker gate passes (no false positives from unrelated work mentions on failure pages). **Manual cleanup needed**: user has 5+ duplicate Hypnotic_Claim drafts on AO3 from the failed retries; either delete them via AO3's drafts list or let AO3 auto-expire them.)
**Earlier:** 2.20.0 (**Feature: Regenerate-all-stories.** Bulk rebuild of every story's derived format files (Markdown chapters, BBCode, Clean HTML, SoFurry HTML, Styled HTML full+chapters, SquidgeWorld, EPUB, optional PDF) from each `MASTER.md`. Two surfaces. **(1) Editor button** — `↻ Regenerate All` in the story-list header next to + Create New Story / Import. Click opens overlay with Skip PDF checkbox (default on); Start kicks off the run and the overlay flips to a live log: per-story stamps, pass/partial/fail counts, progress bar, elapsed timer. SSE-streamed (`/api/editor/regenerate-all/stream/{run_id}`) so updates are instant. Cancel button asks for graceful stop (current story finishes, loop exits); closing the dialog mid-run prompts "leave it running in background?". **(2) Diagnostics test** — `archive.regenerate.all_stories` in Settings → Diagnostics → Archive. Destructive (opt-in per-test confirm), `skip_pdf=True` always so the suite stays inside the 15-min timeout. Reports per-story pass/partial/failed counts + failures list. **Architecture**: 4 new endpoints in `routes/editor_api.py` (`POST /regenerate-all` start, `GET /regenerate-all/active` reattach probe, `GET /regenerate-all/stream/{run_id}` SSE with 15s heartbeats + event-buffer backfill, `POST /regenerate-all/cancel/{run_id}` graceful stop). Thin orchestrator design — bulk runner calls the existing per-story `regenerate()` handler in-process, so per-story behaviour stays the single source of truth and the working regen endpoint is untouched. Global threading.Lock prevents two bulk runs at once (409 with active run_id). Frontend addition lives in `frontend/js/editor.js:renderStoryList()` + new helper `_streamRegenAll(runId)`. **Deferred follow-up**: `m_x/Scripts_Utils/regenerate_all_stories.py` CLI wrapper that loops over `Complete_Stories/` and calls the existing per-story `regenerate_story.py` — lives in a separate repo, didn't ship with this PawPoller version.)
**Previous version:** 2.19.3 (**Fix: AO3 cookie-only posting + Diagnostics cleanup pass.** Real production bug: the Publish matrix at `routes/editor_api.py:get_publish_matrix` had `PLATFORM_CREDS["ao3"] = ("ao3_username", "ao3_password")` hard-coded, so users on the cookie-only auth path (the recommended setup from 2.18.8 onward, to avoid AO3's per-IP login throttle on datacenter IPs) saw "No credentials configured" on every AO3 cell despite their cookie + poller + importer all working fine. Fix: `PLATFORM_CREDS` now supports OR-of-ANDs — either a flat tuple of required keys, or a tuple of groups where any group's keys all-set satisfies. AO3 becomes `(("ao3_username", "ao3_password"), ("ao3_session_cookie",))`. Posting itself (`posting/platforms/ao3.py`) was already cookie-aware; this was purely a UI gate. Plus a cleanup pass over the Diagnostics suite based on the post-2.19.2 Run All skip list: `platforms.ao3.auth/discovery` accepts cookie-only setups (dropped `ao3_username` from `requires_creds`, passes `target_user` + `proxy_kwargs`, switched discovery to `get_all_work_ids()`); `external.github.latest_release` uses `github_pat` from settings as a Bearer token when set, treats 404-without-PAT as the private-repo case; `notifications.digest.builder` replaced with `notifications.digest.data_fetch` that exercises real `_get_digest_deltas()` + `_get_platform_totals()` across all 10 polling platforms; `external.turnstile.reachable` description rewritten to explain what Turnstile actually is. **Tunnel parked:** Cloudflare Tunnel `pawpoller` (Tunnel ID `1e7afca9-e4e0-49d6-8fdc-b9abcf15f3f5`) was created during this session for an aborted Turnstile setup — `cloudflared` is running as a systemd service on the VM but the tunnel has no public hostname configured (requires a Cloudflare-managed domain, which the user doesn't have). It's idle, costs nothing, and exposes nothing. Pick up later if/when you want a domain.) Cleanup pass over the skip list from the post-2.19.2 Run All. `platforms.ao3.auth/discovery` now accepts cookie-only setups: dropped `ao3_username` from `requires_creds` (the username is optional when `ao3_session_cookie` is set per 2.18.8), passes `target_user` to AO3Client (was missing — 3rd positional), passes `proxy_kwargs(s, 'ao3')`, switched discovery to real `get_all_work_ids()`. `external.github.latest_release` now uses `github_pat` from settings as a Bearer token when set; without a PAT, 404 is treated as the "private repo" case with a skip message that explicitly says so. `notifications.digest.builder` replaced with `notifications.digest.data_fetch` — old test chased a `build_digest_text` helper that doesn't exist; new test exercises the actual read-only helpers `_get_digest_deltas()` + `_get_platform_totals()` across all 10 polling platforms to confirm the digest's SQL still executes against the live schema. `external.turnstile.reachable` description rewritten to explain what Turnstile is (Cloudflare's CAPTCHA replacement for the dashboard login). Behaviour of the Telegram + toast tests unchanged — they remain destructive, opt-in.)
**Earlier:** 2.19.2 (**Fix: Diagnostics platform test definitions — second round.** First post-2.19.1 Run All on the server (where vault creds were now visible) unmasked four more bugs in how the platform tests called their clients — none are bugs in the platforms themselves. SqW tests imported `SqWClient` but the class is `SquidgeWorldClient`, and its constructor needs `target_user` as a third positional; discovery now calls `get_all_work_ids()`. DA tests constructed `DAClient` without the required `target_user` positional; `validate_cookies()` takes no args; discovery uses `get_all_deviation_ids()`; DA is a REQUIRED-proxy platform, now receives `**proxy_kwargs(settings, "da")`. SF auth failed with "session did not validate" because the test constructed `SoFurryClient` without CF Worker proxy creds (SF needs proxy on server) — now passes `display_name` and `**proxy_kwargs(settings, "sf")` plus calls `ensure_logged_in()` before `validate_session()` to mirror the poller's session-warming flow. SF discovery: same proxy fix, switched to real method `get_all_gallery_ids()`. WS discovery: switched to real method `get_all_gallery_ids()`. Verification on next Run All: errored / failed counts should drop further; remaining failures should only be legitimate platform issues or genuine cred-missing skips.)
**Earlier:** 2.19.1 (**Fix: Diagnostics suite test definitions.** First live "Run All" from the new Diagnostics tab on the server surfaced four defects in test code (not the subsystems under test): `platforms.wp.auth/discovery` and `platforms.ik.auth/discovery` constructed `WPClient()` / `IKClient()` without their required `target_user` positional arg (and called `validate_user(target)` with an arg the method doesn't accept) — fixed to pass target via constructor and call the no-arg form, also switched discovery to the correct `get_all_story_ids` / `get_all_content_ids` methods; `infra.vault.crypto` assumed `_encrypt_vault(payload)` returned a blob — the real signature is `_encrypt_vault(creds: dict) -> None` that writes to `VAULT_PATH` and `_decrypt_vault() -> dict` that reads from it, so the test now monkeypatches `config.VAULT_PATH` to a tempfile for the round-trip and restores it in finally, skipping cleanly when the vault key isn't reachable; `external.github.latest_release` now treats 404 as a clean skip (repo has no published releases yet) instead of a failure; `archive.pawsync.dry_run` now recognises `Archive root not found` / `no such file` in pawsync stderr as an environment-mismatch skip rather than a failure. Expected outcome on next Run All: 0 errored, 0 failed; skipped goes up by 1-3.)
