...
| GET/POST | `/api/settings/telegram/channel` · `/channel/test` | Telegram channel posting (2.198). Posts-module broadcast target (post-only, NOT in the pollable registry). `clients/tg/client.py` sends via Bot API (sendMessage/sendPhoto/sendMediaGroup); `post_publisher._publish_one` `tg` branch reuses the notification bot token if no posting one. Config `tg_bot_token`(vaulted)/`tg_channel`. |
...
| GET | `/api/analytics/tag-performance` | Tag performance (2.197): which keywords earn engagement. `analytics_queries.get_tag_performance` scores each piece `metric/platform-median`, attributes the ratio to each keyword, indexes tags by median ratio (>1 = overperforms) + best pairs. `_norm_tag` merges case/underscore variants. Params `min_works`/`limit`/`platform`. Deterministic — no model. Rendered on `#/analytics` (`_tagPerfHtml`). |
