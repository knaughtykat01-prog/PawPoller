...
| AB | **Achievement-style ERROR popups + send/report action** — errors surface like the Laurels popups (friendly card, full detail expandable) with "Copy report" + a send-to action | 🔴 | Rhys 2026-07-19 — his sentence cut off at "a way to send it to…"; assumed Telegram (alerts already pipe there) — CONFIRM destination. Also: tag-format display at post time ("will post as: …") folded here as popup polish |
