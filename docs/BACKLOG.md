...
| # | Item | Status | Notes |
|---|------|--------|-------|
| S | **Masterpiece VARIANTS + XMB Showcase** (Rhys 2026-07-19: variants "each tracked in its stats but part of the one cohort"; PS3-style animated browse) | 🟡 spec'd + mockup | Spec `docs/specs/masterpiece_variants.md` (variant list in masterpiece.json + `variant_key` on members + filtered rollups; dup-finder gains "Same piece, different variants"; `#/showcase` XMB view). Interactive mockup with real art: `C:\Users\rhysc\claude\art_audit\xmb_mockup.html`. Companion §4: import the ~130 new collection pieces as Masterpieces once variants land. AWAITING Rhys's look at the mockup → then build Phases 1-4 |
