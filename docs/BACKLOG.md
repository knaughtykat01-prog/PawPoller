...
| G3 | **Reply-to-comments inbox** — unified `#/inbox` across personas | ⚪ **AUDITED 2026-07-23 — cheaper than feared** | 17-platform sweep done (spec §2.3 has the full matrix). IB+FA comment CONTENT is already stored w/ threading → **Stage A0 (S/M): inbox over IB+FA now** + handled-flag. **A1 (M):** cheap official adds bsky/mast/e621/da into one `platform_comments` table. **B (M):** native reply bsky→mast→e621 (mast needs the write-scope token; DA is NOT read-only — official comment-post needs `comment.post` scope). Scrape platforms (ws/sf/sqw/ao3/tw) stay "reply on site ↗". Say the word to build A0 |
