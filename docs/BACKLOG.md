...
| AA | ~~**Retro "2005 web" theme** — early-2000s style: beveled buttons, gradient headers, boxy tables, Verdana energy~~ | 🟢 **DONE 2.172** | `[data-theme="retro_2005"]` in tokens.css: silver/beige chrome + white cards + web-blue accent + near-0 radii + Verdana/Tahoma + hard shadows, plus theme-scoped component rules (beveled buttons, blue gradient page-header title bars, boxy blue-header tables, underlined blue/purple links). Registered in `app.js` THEMES + `routes/api.py` theme allowlist. Settings → Appearance |
