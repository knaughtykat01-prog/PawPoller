...
_Last updated: 2026-07-30 (after 2.193.0 — unified art detail; 2.192.0 — self-comment detection). **All open feature
items shipped** — only deferred cleanup (L1/L2), the multi-user/roles park, and the art-audit tail remain. NB: the
"last updated" line lagged nine versions (2.172 → 2.191) before this — bump it with the rows._

Legend: 🔴 open · 🟡 in progress · 🟢 done · ⚪ deferred/parked

---

## 🟢 Latest asks (2026-07-30)

| # | Item | Status | Notes |
|---|------|--------|-------|
| SC | ~~**Own comments counted as new comments** ("comments are also looping in our own comments as new … should detect any comments made by the posting account is not tracked as a new comment")~~ | 🟢 **DONE 2.192.0** | Rhys chose *store it, flag it, exclude it* over dropping the row. New `polling/self_comment.py`; `is_own` on all three comment tables. **IB and FA had NO filter at all** → own comments toasted, pushed to Telegram, counted in `new_comments_found`, sat in the Inbox as "to answer", and ranked the posting account as its own **top fan**. The four A1 platforms' filter was actively *wrong*: `split("@")[0]` on both sides meant a Mastodon `@rhys@other.instance` matched our `@rhys@our.instance`. Now full-handle matching with OUR side widened for Mastodon's `acct` format; bsky's email-capable `bsky_identifier` discarded when it contains `@`. Own rows stay in the Inbox as thread context but report handled. `remember_own_handle` persists `<code>_own_handle` (plaintext — not a secret) since mast/bsky only know their handle at login. `backfill_own_comments()` retro-flags; runs once per process on first `GET /api/inbox` + `POST /api/inbox/backfill-own`. **Known limitation:** `milestone_comments` + `total_comments` read the platform's own server-side count, which includes your replies — unreachable by any local flag. +13 tests (first ever coverage of `inbox_capture.py`) |
| UD | ~~**Artwork vs Masterpieces open two different pages** ("they should be unified but when its say a variant peice, it should come up with that variant peice looped in with the rest")~~ | 🟢 **DONE 2.193.0** | Rhys chose *both routes integrated* (no redirect). They were never two entities — `masterpiece.json` is a back-compat superset of `artwork.json`, both endpoints load via `artwork_reader.load_artwork`, and there is no discriminator in the data. Merged onto the Masterpiece renderer (the superset); `Artwork.renderDetail` delegates, old body kept unreachable as `_renderDetailLegacy` for one release. Ported in the only four Artwork-only capabilities: publish now, schedule, delete, alt text. Variant deep-link `?v=<key>` (query tail, NOT a path segment — artwork names may contain `/`); opening a variant selects that render **with the sibling strip still shown**. `bookshelf.js` variant tiles now carry the key — before this every tile opened the master hero. Also fixed two latent bugs: the `.art-plat-row`/`artwork-plat-row` selector mismatch meant "already-posted → dim/disable" had **never** fired, and per-platform Override-tags inputs were rendered but never read. +7 tests (backend only — no JS harness exists, so the merged renderer is unverified by tests) |

| LOG | ~~**Access tokens printed in the logs** ("add the filter please. It shouldnt be able to be found")~~ | 🟢 **DONE 2.193.1** | Found while scanning the 2.193.0 deploy output on prod — not deliberate logging: **httpx logs the full request URL at INFO** and Threads/IG/Tumblr put the token in the query string, Telegram in the *path*. Live creds were in `server.log`, `docker logs` and the dashboard Logs view. New `log_redaction.py` from all three entry points: **pattern** layer (sensitive params, telegram bot path, Bearer/Token, Authorization/Cookie) + **value** layer (real vault secrets, so a token is unfindable in ANY shape). Redacts `record.args` not just `msg` (httpx puts the URL in args — msg-only would have caught nothing); handler-scoped not logger-scoped (root-logger filters skip propagated library records); thread-local recursion guard (reading settings can log); never raises/drops; identity fields + values <8 chars exempt. +13 tests. **⚠ Pre-2.193.1 log files still hold live tokens — rotate them.** |

Spec for SC + UD: `docs/specs/self_comments_and_unified_detail.md`. All three **deployed to prod**; none tagged
(newest published release remains `v2.182.0`, so the desktop update path is untouched).
...
| AVU | ~~**Add-variant upload button on a Masterpiece** (Rhys ask, 2026-07-24)~~ | 🟢 **DONE 2.190.2** | `POST /{name}/variants/upload` (multipart) + ＋ Add variant button in the detail hero — upload a fresh render straight in as a labeled variant (key derived+uniquified). |
| IKT | ~~**Itaku posting auth-token field** (live error, 2026-07-26)~~ | 🟢 **DONE 2.191** | IK settings only had a username box (“no auth required”) but posting needs `ik_auth_token`. Added the token input to the connect form + a Save-token control on the connected panel; `/auth/token` endpoint; status shows posting-readiness. |
