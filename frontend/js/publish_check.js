...
window.PublishCheck = (function () {
    ...
    function _render(data) {
        ...
        for (const row of data.matrix) {
            ...
            html += '<tr><td class="ch-col">' + chLabel + '</td>';
            for (const p of data.platforms) {
        ...
        body.querySelectorAll('.publish-check-cell').forEach(td => {
            td.addEventListener('click', () => {
                ...
                _showDetail(JSON.parse(td.dataset.cell), td.dataset.platName, td.dataset.chTitle);
            });
        });
    }
    ...
    function _showDetail(cell, platName, chTitle) {
