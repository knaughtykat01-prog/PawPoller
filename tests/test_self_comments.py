...
def test_normalise_handle_trims_cases_and_one_leading_at():
    assert self_comment.normalise_handle("  @Rhys  ") == "rhys"
    assert self_comment.normalise_handle("RHYS") == "rhys"
    ...
    assert self_comment.normalise_handle("@rhys@ours.social") == "rhys@ours.social"
...
def test_mastodon_other_instance_is_not_us():
    conn = get_connection()
    try:
        config.save_settings({"mast_own_handle": "rhys@ours.social"})
        ...
        assert self_comment.is_own_author("rhys@some.other.instance", handles) is False
        assert self_comment.is_own_author("@rhys@other.social", handles) is False

        # Us, fully qualified.
        assert self_comment.is_own_author("rhys@ours.social", handles) is True
        ...
        assert self_comment.is_own_author("rhys", handles) is True
    finally:
        conn.close()
...
def test_bsky_email_login_is_not_treated_as_a_handle():
    ...
    conn = get_connection()
    try:
        config.save_settings({"bsky_identifier": "rhyscharlie@gmail.com"})
        handles = self_comment.own_handles(conn, "bsky")
        assert "rhyscharlie@gmail.com" not in handles
        # An email must not make every commenter named "rhyscharlie" us.
        assert self_comment.is_own_author("rhyscharlie", handles) is False

        # A real handle (no '@') is trusted.
        config.save_settings({"bsky_identifier": "rhys.bsky.social"})
        handles = self_comment.own_handles(conn, "bsky")
        assert self_comment.is_own_author("rhys.bsky.social", handles) is True
    finally:
        conn.close()
...
def test_own_handles_reads_the_platform_identity_keys():
    conn = get_connection()
    try:
        config.save_settings({"username": "kithetiger", "fa_username": "KitheTiger",
                              "e621_username": "kithe", "da_target_user": "KitheArt"})
        assert self_comment.is_own_author(
            "kithetiger", self_comment.own_handles(conn, "ib")) is True
        assert self_comment.is_own_author(
            "kithetiger", self_comment.own_handles(conn, "fa")) is True
        assert self_comment.is_own_author(
...
def test_ib_own_comment_stored_flagged_and_stranger_untouched():
    conn = get_connection()
    try:
        config.save_settings({"username": "kithetiger"})
        conn.execute("INSERT INTO submissions (submission_id, title)"
                     " VALUES (555, 'My IB Piece')")
        conn.commit()

        assert _seed_ib(conn, 1001, "kithetiger") is True   # ours — still STORED
...
def test_fa_batch_flags_ours_only():
    conn = get_connection()
    try:
        config.save_settings({"fa_username": "KitheTiger"})
        ...
        fa_queries.upsert_fa_comments_batch(conn, 0, [
            {"comment_id": "c1", "submission_id": 777, "username": "kithetiger",
...
def test_own_platform_comment_is_context_not_a_task():
    conn = get_connection()
    try:
        _seed_platform(conn, "r1", "rhys.bsky.social", True)
...
def test_unhandled_badge_ignores_own_comments():
    ...
    try:
        for i in range(3):
            _seed_platform(conn, f"mine{i}", "rhys.bsky.social", True)
...
def test_backfill_flags_historical_rows_and_is_idempotent():
    ...
    try:
        ...
        _seed_ib(conn, 1001, "kithetiger")
        _seed_ib(conn, 1002, "ibfan")
        _seed_platform(conn, "r1", "rhys.bsky.social", False)
        conn.commit()
        assert conn.execute(
        ...
        config.save_settings({"username": "kithetiger",
                              "bsky_own_handle": "rhys.bsky.social"})
