...
async def fn_connect(body: dict):
    """Validate a FurryNetwork email + password (OAuth password grant) and save
    them along with the obtained refresh/access tokens."""
    email = str(body.get("username", "") or "").strip()
    password = str(body.get("password", "") or "").strip()
    if not email or not password:
        raise HTTPException(400, "Both your FurryNetwork email and password are required")

    from clients.fn.client import FnClient, FnAuthError
    client = FnClient(username=email, password=password)
    try:
        await client.login()
    ...
    if not name:
        raise HTTPException(401, "Auth failed — check your FurryNetwork email and password.")

    config.save_settings({
        "fn_username": email, "fn_password": password,
        "fn_refresh_token": client.refresh_token, "fn_access_token": client.access_token,
...
async def trigger_fn_poll():
    try:
        spawn(run_fn_poll_cycle(), "run_fn_poll_cycle")
...
async def fn_full_resync():
    try:
        spawn(run_fn_poll_cycle(force_full=True), "run_fn_poll_cycle full-resync")
        return {"status": "started"}
    except Exception as e:
