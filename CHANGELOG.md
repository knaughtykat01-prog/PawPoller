...
> needed (Furbooru's data is public); an optional API key raises the rate limit. PawPoller then tracks your own uploads'
> score, favourites and comments, with the same dashboard, posts grid, per-piece charts, comparison and CSV export as
> every other platform. It also feeds the weekly digest, tag performance and repost radar. Posting to Furbooru isn't
> available yet — this is analytics only for now.

Second platform of the expansion (see `docs/specs/platform_expansion_v2.md`), and the first of the **booru** family.
Furbooru runs **Philomena** (same engine as Derpibooru), whose read JSON API is public — so the client is written against
`base_url` and is reusable for the whole Philomena family. Furbooru uses the **SCORE model** (upvotes − downvotes, may be
negative), identical in shape to e621, so the backend was generated by mechanical transform of the e621 files and the
frontend mirrors e621's score dashboard. Poll-only for now (posting would need a cookie-scrape upload, deferred exactly
as e621 posting was).

- **Client** `clients/fbr/client.py` — `FurbooruClient(username, api_key, base_url)`, paginated discovery
  (`search/images?q=uploaded_by:{user}`), `_parse_image` → normalised score/up/down/faves/comments/rating-from-tags,
  hotlinkable thumbnails, `validate_session` (username resolves). Reusable across Philomena boorus via `base_url`.
- **DB** `database/fbr_schema.sql` + `fbr_queries.py` — `fbr_submissions`/`fbr_snapshots`/`fbr_poll_log` (SCORE model),
  full query surface; `account_id` carried in the fresh schema (avoids the schema-load-before-migration gotcha).
- **Poller** `polling/fbr_poller.py` — `run_fbr_poll_cycle`, wired into the orchestrator + `multi_account` registry.
- **API** `routes/fbr_api.py` (`/api/fbr/*`) mounted; session-check registered (username-only "configured" gate);
  connect takes username (required) + api_key (optional).
- **Frontend** — `platforms.js` registry (`pollOnly:true`), `renderFBRDashboard/Submissions/Detail/Compare` renderers,
  Settings connect form, ~14 API methods, `fbrTopList`/`fbrSubmissionsTable`/`fbrPollLogTable` components. Furbooru flows
  into digest/insights/tag-perf/repost-radar via the platform maps (`telegram.py`, `analytics_queries.py`).
- **Config** — `fbr` credential fields (username plaintext; API key vaulted).

**Also fixed (pre-existing FurryNetwork gaps found while wiring Furbooru):** `fn` was missing from the dashboard
`_dashResync` map, the Settings "Poll all"/"Resync all" `TRIGGERS`/`RESYNCS` maps, the Platforms-hub `fetchers` map, the
submission-nav `prefixMap`, and `_pollingAuth` — so FN's dashboard "Full Resync", the hub-tile headline stat, and the
bulk poll/resync could throw or under-populate once FN was configured. Added both `fn` and `fbr` to all of them.

**Tests:** `tests/test_fbr.py` (4) — Philomena image → score-model parse, rating-from-tags, paginated discovery +
`validate_session`, and a full poll cycle storing submissions/snapshots. Network faked throughout.
